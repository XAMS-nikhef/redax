...
int Options::Load(std::string name, mongocxx::collection* opts_collection, std::string override_opts) {
  ...
  for (auto doc : opts_collection->aggregate(pl)) {
    ...
    try{
      fDetector = bson_options["detectors"][fHostname].get_string().value.to_string();
...
std::string Options::GetString(std::string path, std::string default_value){
  try{
    return bson_options[path].get_string().value.to_string();
  }
  catch (const std::exception &e){
    //LOG
    fLog->Entry(MongoLog::Local, "Using default value for %s", path.c_str());
...
std::vector<BoardType> Options::GetBoards(std::string type){
  ...
  if(type == "V17XX")
    types = {"V1724", "V1725", "V1730", "V1724_MV", "f1724"};
  else if (type == "V27XX")
    types = {"V2718", "f2718"};
  else if (type == "V1495")
  ...
  for(bsoncxx::array::element ele : subarr){
    std::string btype = ele["type"].get_string().value.to_string();
    if(!std::count(types.begin(), types.end(), btype))
      continue;
    try{
      if(ele["host"].get_string().value.to_string() != fHostname)
        continue;
    }
    catch(const std::exception &e){
    ...
    bt.type = ele["type"].get_string().value.to_string();
    bt.vme_address = DAXHelpers::StringToHex(ele["vme_address"].get_string().value.to_string());
    ret.push_back(bt);
  }
...
std::vector<RegisterType> Options::GetRegisters(int board, bool strict){
  ...
  for(bsoncxx::array::element ele : regarr){
    try{
      ...
      try{
        sdet = ele["board"].get_string().value.to_string();
        ibid = -1;
      }catch(const std::exception& ee){
    ...
    if ((ibid == board) || (sdet == fDetector) || (sdet == "all")) {
      RegisterType rt;
      rt.reg = ele["reg"].get_string().value.to_string();
      rt.val = ele["val"].get_string().value.to_string();
...
int Options::GetHEVOpt(HEVOptions &ret){
  try{
    ...
    ret.address = bson_options["DDC10"]["address"].get_string().value.to_string();
    ret.required = bson_options["DDC10"]["required"].get_string().value.to_string();
  }catch(std::exception &E){
