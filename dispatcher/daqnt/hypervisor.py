...
#         :param logger: logger
#         :param config: the dispatcher-like config of what this HV controls
#         :param vme_crates: a dict of {number: address} VME crates to control
#         :param detector: detector (either of 'tpc', 'muon_veto', or 'neutron_veto')
...
#         self.hosts = config['controller'] + config['readers']
#         self.vme_crates = vme_crates
#         self.sh = daqnt.SignalHandler() if sh is None else sh
#         self.event = self.sh.event
...
#         self.sleep(0.2)
#         for i in range(5):
#             if self.db.control.find_one({'_id': oid})['acknowledged'][cc] == 0:
...
#             cmd = f'ssh xedaq@{physical_host} "source /daq_common/etc/daqrc ; {reset} cd /daq_common/daqnt/readers && ./start_process.sh --process {process} --id {id_}{test}"'
...
#             physical_host, process, id_ = h.split('_')
#             cmd = f'ssh xedaq@{physical_host} "screen -X -S {process}{id_} quit"'
#             cp = subprocess.run(cmd, shell=True, capture_output=True)
#             ret.append(cp.returncode)
#             if cp.stdout:
#                 self.logger.debug(cp.stdout.decode())
...
#     def tactical_nuclear_option(self):
#         """
...
#         URL = (f"https://xenonnt.lngs.infn.it/api/{endpoint}?"
#                f"api_user={os.environ['DAQ_API_USER']}&api_key={os.environ['DAQ_API_KEY']}")
...
#         else:
#             # TODO calling API only works if not in linked mode!
#             self.call_api(
#                 endpoint=f'setcommand/{self.detector}',
#                 data={'active': 'false', 'softstop': 'false'})
...
# def main(client, daqnt_bot):
#     db = client.daq
#     logger = daqnt.get_daq_logger('main', 'hypervisor',
#                                   level="DEBUG",
#                                   mc=client)
#     config = {'controller': ['reader0_controller_0'],
#               'readers': [f'reader{i}_reader_0' for i in range(3)]}
#     vme = {str(c): f'192.168.131.6{c}' for c in range(5)}

#     try:
#         hyperv = Hypervisor(db, logger=logger, config=config, vme_crates=vme, detector='tpc', slackbot=daqnt_bot)
#         hyperv.ensure_readout_is_up()
#     except Exception as e:
#         print(type(e), e)
...
#         print('Usage: ' + sys.argv[0] + " <uri> [<daq_slack_api>]")
#         sys.exit(1)
#     if len(sys.argv) == 3:
#         daqnt_bot = daqnt.DaqntBot(sys.argv[2])
#     else:
#         daqnt_bot = None
#     with MongoClient(sys.argv[1]) as client:
#         main(client, daqnt_bot)
